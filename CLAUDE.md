...
- `/position-paper` is its public bibliographic landing page; `/position-paper/read`

  is the permanent, indexed full-text reader.

- The final Markdown, reader routes and EPUB belong in the repository and normal

  production builds. There is no longer a reader on/off toggle.

- Use `npm run dev` for development. Use `npm run export:epub` after changing the

  text or metadata, and include the regenerated EPUB. `npm run build:paper` exports

  and builds together (Python 3 required; `EPUB_PYTHON` may specify the executable).

- Keep reader CSS imported only by `PositionPaperReader.svelte` so other routes

  do not load its styles or Atkinson Hyperlegible font.

- The corrected publisher PDF has not yet been supplied. Keep `pdfAvailable`
